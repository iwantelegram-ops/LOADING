...
from pyrogram.types import Message as _Message, ChatMemberUpdated as _ChatMemberUpdated, ChatPrivileges as _ChatPrivileges
...
async def _check_vc_manage_permission(chat_id: int) -> bool:
    ...
    try:
        ...
        if _is_chat_invalid_error(e):
            # Chat sudah tidak ada (kemungkinan besar grup dihapus) — bukan
            # error sementara, retry tidak akan pernah berhasil. Nonaktifkan
            # Security OS grup ini agar scheduler tidak terus mencoba.
            await _handle_chat_invalid(chat_id, f"{type(e).__name__}: {e}")
            return False
        # Paksakan selalu True jika terjadi error apa pun (termasuk error
        # sementara seperti network/FloodWait)
        print(f"[UB-VC] Gagal cek izin manage_video_chats grup={chat_id}: {e} — paksa OK (fail-open)")
        has_perm = True

    # SEKRING TAMBAHAN: Pastikan variabel has_perm SELALU True agar siklus scan tidak pernah di-skip!
    has_perm = True
...
# ══════════════════════════════════════════════════════════════════════════════
# AUTO-SETUP USERBOT — Otomatisasi join + promote userbot, lalu invite bot
# pemantau, supaya admin grup tidak perlu kerja manual sama sekali.
# ══════════════════════════════════════════════════════════════════════════════
#
# URUTAN (setiap langkah WAJIB diverifikasi sebelum lanjut ke langkah
# berikutnya — tidak ada langkah yang "dianggap berhasil" tanpa konfirmasi
# ulang dari Telegram, sama seperti prinsip workshop_join_pool.py):
#
#   1. Userbot member grup?
#        Belum → coba add_chat_members() via bot utama (berhasil kalau
#        userbot sudah resolvable / grup cukup terbuka).
#        Gagal/tidak resolvable → generate invite link via bot utama,
#        userbot join_chat() pakai link itu sendiri (paling pasti untuk
#        grup privat — userbot yang join, bukan di-add).
#      → VERIFIKASI get_chat_member() sebelum lanjut.
#
#   2. Userbot admin grup (manage_video_chats + invite_users)?
#        Belum → bot utama promote_chat_member() userbot.
#        (Bot utama WAJIB sudah admin dgn hak "Add New Admins"/can_promote_members
#        di grup itu — kalau tidak, promote akan gagal dan proses berhenti
#        di sini dengan instruksi manual.)
#      → VERIFIKASI is_userbot_admin() (cache diinvalidasi dulu) sebelum lanjut.
#
#   3. Bot pemantau member grup?
#        Belum → userbot KIRIM /start ke bot pemantau via @username dulu
#        (userbot belum pernah "ketemu" bot pemantau → belum resolvable →
#        add_chat_members dengan ID mentah akan PEER_ID_INVALID).
#        Setelah resolvable → userbot add_chat_members() bot pemantau
#        (userbot sudah admin+invite_users dari langkah 2, jadi ini berhasil).
#      → VERIFIKASI get_chat_member() bot pemantau.
#
# Kalau langkah manapun gagal, fungsi berhenti di situ dan mengembalikan
# instruksi manual yang presisi (mulai dari langkah yang gagal saja, bukan
# dari awal) — admin grup tidak perlu mengulang langkah yang sudah berhasil.
#
# CATATAN PENTING — kenapa langkah 3 harus lewat userbot, bukan bot utama:
#   Telegram TIDAK mengizinkan bot menambahkan bot lain ke grup. Userbot
#   adalah akun biasa (bukan bot), jadi dia BISA invite bot pemantau —
#   ini sebabnya bot pemantau harus diundang oleh userbot, bukan bot utama,
#   dan ini sebabnya urutan join→admin userbot HARUS selesai dulu sebelum
#   langkah 3 dijalankan (lihat juga core/workshop_join_pool.py yang
#   menemukan constraint ini lebih dulu untuk kasus serupa).
# ══════════════════════════════════════════════════════════════════════════════

async def ensure_userbot_in_group_and_promote(
    chat_id: int,
    inviter_bot: _Client,
    monitor_bot_id: int = 0,
    monitor_username: str = "",
) -> tuple[bool, list[str]]:
    """
    Otomatisasi penuh: pastikan userbot jadi member → admin grup, lalu
    (jika monitor_bot_id diisi) userbot meng-invite bot pemantau juga.

    Dipanggil setelah setup_monitor_bot() menyimpan token bot pemantau,
    supaya admin grup tidak perlu menambahkan siapa pun secara manual.

    Return: (semua_langkah_berhasil: bool, log_langkah: list[str])
    Setiap entri log_langkah berisi status 1 langkah (✅/⚠️/❌) untuk
    ditampilkan ke admin — baik saat sukses penuh maupun saat berhenti
    di tengah jalan.
    """
    log: list[str] = []

    if not userbot or not _ub_ready:
        return False, ["❌ Userbot belum online — proses otomatis dibatalkan."]
    ...
    # ── Langkah 1: pastikan userbot member grup ──────────────────────────────
    try:
        member = await inviter_bot.get_chat_member(chat_id, ub_id)
        is_member_already = member.status not in (
    ...
    if is_member_already:
        log.append("✅ Userbot sudah member grup — lanjut.")
    else:
        joined = False

        # 1a. Coba add_chat_members lewat bot utama dulu (jalan kalau userbot
        #     sudah resolvable/grup cukup terbuka untuk di-add langsung).
        try:
            await inviter_bot.add_chat_members(chat_id, ub_id)
            joined = True
        except Exception as e:
            print(f"[UB-AutoSetup] add_chat_members userbot gagal chat={chat_id}: {e} — coba invite link.")

        # 1b. Fallback: generate invite link via bot utama, userbot join sendiri.
        #     Ini cara paling pasti untuk grup privat — tidak bergantung pada
        #     userbot sudah resolvable oleh bot utama atau belum.
        if not joined:
            try:
                link = await inviter_bot.export_chat_invite_link(chat_id)
                await userbot.join_chat(link)
                joined = True
            except Exception as e:
                log.append(
                    "❌ <b>Gagal mengundang userbot ke grup secara otomatis.</b>\n"
                    f"   ({type(e).__name__}: {e})\n"
                    "   Tambahkan userbot ke grup secara manual, lalu jadikan admin\n"
                    "   dengan izin <code>Kelola Obrolan Video</code> + <code>Undang Pengguna</code>,\n"
                    "   kemudian aktifkan ulang Security OS."
                )
                return False, log

        # ── VERIFIKASI — jangan anggap berhasil tanpa cek ulang ──────────────
        try:
            member = await inviter_bot.get_chat_member(chat_id, ub_id)
            verified = member.status not in (ChatMemberStatus.LEFT, ChatMemberStatus.BANNED)
        except Exception:
            verified = False

        if not verified:
            log.append(
                "❌ <b>Userbot tidak terkonfirmasi masuk grup setelah diundang.</b>\n"
                "   Tambahkan userbot ke grup secara manual, lalu jadikan admin\n"
                "   dengan izin <code>Kelola Obrolan Video</code> + <code>Undang Pengguna</code>,\n"
                "   kemudian aktifkan ulang Security OS."
            )
    ...
    already_admin = await is_userbot_admin(chat_id)

    if already_admin:
        log.append("✅ Userbot sudah admin grup — lanjut.")
    else:
        try:
            await inviter_bot.promote_chat_member(
                chat_id=chat_id,
                user_id=ub_id,
                privileges=_ChatPrivileges(
            ...
            log.append(
                "❌ <b>Gagal mengadminkan userbot otomatis.</b>\n"
                f"   ({type(e).__name__}: {e})\n"
                "   Pastikan bot utama adalah admin dengan hak <code>Tambah Admin Baru</code>\n"
                "   di grup ini, lalu jadikan userbot admin secara manual dengan izin\n"
                "   <code>Kelola Obrolan Video</code> + <code>Undang Pengguna</code>."
            )
            return False, log

        # ── VERIFIKASI — cek ulang status admin, jangan percaya promote saja ──
        invalidate_ub_admin_cache(chat_id)
        verified_admin = await is_userbot_admin(chat_id)
        if not verified_admin:
            log.append(
                "❌ <b>Userbot tidak terkonfirmasi sebagai admin setelah dipromosikan.</b>\n"
                "   Jadikan userbot admin secara manual dengan izin\n"
                "   <code>Kelola Obrolan Video</code> + <code>Undang Pengguna</code>."
            )
            return False, log
    ...
    if monitor_bot_id:
        try:
            mon_member = await inviter_bot.get_chat_member(chat_id, monitor_bot_id)
            monitor_already_in = mon_member.status not in (
                ChatMemberStatus.LEFT, ChatMemberStatus.BANNED,
            )
        except Exception:
            monitor_already_in = False

        if monitor_already_in:
            log.append("✅ Bot pemantau sudah member grup — lanjut.")
        else:
            # 3a. WAJIB: userbot belum pernah "ketemu" bot pemantau → Telegram
            #     belum punya access_hash-nya di sisi userbot → add_chat_members
            #     pakai monitor_bot_id mentah akan gagal PEER_ID_INVALID.
            #     Userbot kirim /start ke bot pemantau lewat @username dulu —
            #     ini membuatnya resolvable (sama seperti pola Bengkel kirim
            #     DM ke bot utama di workshop_join_pool.py).
            if monitor_username:
                try:
                    await userbot.send_message(f"@{monitor_username}", "/start")
                    await asyncio.sleep(1)  # beri waktu Telegram cache access_hash
                except Exception as e:
                    log.append(
                        "❌ <b>Userbot gagal membuat bot pemantau resolvable (kirim /start gagal).</b>\n"
                        f"   ({type(e).__name__}: {e})\n"
                        "   Tambahkan bot pemantau ke grup secara manual — bot akan\n"
                        "   dikenali otomatis saat masuk."
                    )
                    return True, log
            else:
                log.append(
                    "⚠️ <b>Username bot pemantau tidak tersedia — tidak bisa resolve otomatis.</b>\n"
                    "   Tambahkan bot pemantau ke grup secara manual — bot akan\n"
                    "   dikenali otomatis saat masuk."
                )
                return True, log

            # 3b. Sekarang resolvable — userbot add_chat_members bot pemantau.
            try:
                await userbot.add_chat_members(chat_id, monitor_bot_id)
            except Exception as e:
                log.append(
                    "⚠️ <b>Userbot gagal mengundang bot pemantau secara otomatis.</b>\n"
                    f"   ({type(e).__name__}: {e})\n"
                    "   Tambahkan bot pemantau ke grup secara manual — bot akan\n"
                    "   dikenali otomatis saat masuk."
                )
                # Tidak fatal — userbot+admin sudah berhasil, ini cuma langkah opsional.
                return True, log

            try:
                mon_member = await inviter_bot.get_chat_member(chat_id, monitor_bot_id)
                monitor_verified = mon_member.status not in (
                    ChatMemberStatus.LEFT, ChatMemberStatus.BANNED,
                )
            except Exception:
                monitor_verified = False

            if not monitor_verified:
                log.append(
                    "⚠️ <b>Bot pemantau tidak terkonfirmasi masuk grup.</b>\n"
                    "   Tambahkan secara manual — bot akan dikenali otomatis saat masuk."
                )
                return True, log
...
def _is_chat_invalid_error(exc: BaseException) -> bool:
    """
    True jika exception ini menandakan chat/channel ITU SENDIRI sudah tidak
    valid lagi — grup dihapus permanen, atau userbot sudah sama sekali tidak
    punya akses/peer ke chat itu (bukan soal hak admin, tapi chat-nya sendiri
    sudah tidak bisa di-resolve Telegram).

    BUKAN dipakai untuk error sementara (FloodWait, network) — hanya untuk
    error yang menandakan retry tidak akan pernah berhasil selama chat_id ini
    tetap sama, karena chat-nya sudah tidak ada.

    Dicek lewat nama class (sama prinsipnya dengan _is_session_dead_error)
    supaya tetap terdeteksi walau ada perbedaan exception hierarchy antar
    versi pyrogram/pyrofork.
    """
    name = type(exc).__name__
    if name in ("ChannelInvalid", "ChatIdInvalid", "PeerIdInvalid"):
        return True
    # Fallback ekstra: beberapa versi membungkus pesan mentah Telegram di
    # dalam exception generik — cek substring kode error sebagai jaring kedua.
    msg = str(exc)
    return any(code in msg for code in ("CHANNEL_INVALID", "CHAT_ID_INVALID", "PEER_ID_INVALID"))


async def _handle_chat_invalid(chat_id: int, reason: str) -> None:
    """
    Dipanggil saat terdeteksi sebuah grup yang Security OS-nya aktif TIDAK
    BISA diakses lagi karena chat-nya sendiri sudah invalid (kemungkinan
    besar grup sudah dihapus permanen oleh pemiliknya).

    Tanpa penanganan ini, scheduler 30-menit akan mencoba scan grup ini
    SELAMANYA dan selalu gagal dengan error yang sama — membuang resource
    dan membanjiri log tanpa ada gunanya, karena chat yang sudah dihapus
    tidak akan pernah valid lagi.

    Aksi: nonaktifkan Security OS untuk grup ini di DB (sec_os_set_enabled
    False) — sama seperti kalau admin menekan tombol "Nonaktifkan" manual.
    Dipanggil sekali per kejadian (bukan loop) — tidak ada DM ke owner di
    sini karena ini per-grup, bukan insiden global seperti session dead.
    """
    try:
        await _sec_os_set_enabled(chat_id, False)
        print(
            f"[UB-VC] 🛑 Grup {chat_id}: {reason} — chat tidak valid/sudah dihapus. "
            f"Security OS dinonaktifkan otomatis untuk grup ini agar scheduler "
            f"tidak retry selamanya."
        )
    except Exception as e:
        print(f"[UB-VC] Gagal menonaktifkan Security OS untuk grup invalid {chat_id}: {e}")
...
async def _log_os_action(chat_id: int, user_id: int, kode: str, reason: str) -> None:
    ...
    try:
        ...
        try:
            ...
            # Bot pemantau belum pernah "bertemu" peer ini (lihat kode
            # VIOLATION_VC_MUTE_PEER — bisa terjadi tepat untuk kasus ini).
            # Fallback ke userbot: dia BARU SAJA melihat user ini langsung
            # di VC (GetGroupParticipants), jadi peer sudah ada di cache
            # sesi userbot walau bot pemantau belum kenal sama sekali.
            try:
                if userbot:
                    u2   = await userbot.get_users(user_id)
                    name  = u2.first_name or str(user_id)
                    uname = f"@{u2.username}" if u2.username else f"id:{user_id}"
            except Exception:
                pass

        waktu  = _dt_vc.now(_WIB_VC).strftime("%H:%M:%S · %d %b %Y WIB")

        # Terjemahkan alasan teknis ke kalimat ramah awam
        _reason_map = {
            "bukan member grup":           "Bukan anggota grup ini",
            "bio mengandung link":         "Bio profil mengandung link",
            "profil belum terverifikasi":  "Profil belum diverifikasi oleh bot",
            "bio bersih / tidak ada link": "Bio sudah bersih — tidak ada link",
        }
        reason_display = _reason_map.get(str(reason).lower().strip(), _html_vc.escape(str(reason)))
...
async def setup_monitor_bot(
    ...
    #     pemantau — supaya admin grup tidak perlu kerja manual sama sekali.
    #     Tidak fatal jika gagal di tengah jalan (lihat log) — token bot
    #     pemantau tetap tersimpan dan bisa dilanjutkan manual dari titik gagal.
    try:
        auto_ok, auto_log = await ensure_userbot_in_group_and_promote(
            chat_id, inviter_bot, monitor_bot_id=monitor_bot_id, monitor_username=monitor_uname,
        )
    except Exception as e_auto:
        auto_ok, auto_log = False, [f"❌ Proses otomatisasi error tak terduga: {e_auto}"]
    ...
    if auto_ok:
        return True, (
            ...
            f"<b>Proses otomatis:</b>\n{auto_log_text}"
        )

    return True, (
        f"Bot @{monitor_uname} berhasil dikonfigurasi.\n\n"
        f"<b>Proses otomatis (berhenti di tengah jalan):</b>\n{auto_log_text}"
    )
...
async def check_activation_prerequisites(
    ...
    Syarat WAJIB (memblokir aktivasi):
      0. Chat itu sendiri masih bisa diakses (belum dihapus/invalid)
    ...
    # ── Syarat wajib 0: chat itu sendiri masih bisa diakses ─────────────────
    # Cek ini SEBELUM syarat lain — kalau grup sudah dihapus/tidak valid,
    # tidak ada gunanya cek hak admin atau bot pemantau sama sekali, dan
    # admin tidak boleh bisa "paksa aktifkan" Security OS untuk grup yang
    # sudah tidak ada (akan selalu gagal lagi di scheduler 30 menit nanti).
    try:
        await client.get_chat(chat_id)
    except Exception as e:
        if _is_chat_invalid_error(e):
            blockers.append(
                "❌ <b>Grup ini sudah tidak bisa diakses (kemungkinan sudah dihapus).</b>\n"
                "└ Security OS tidak bisa diaktifkan untuk grup yang sudah tidak ada.\n"
                "   Jika ini keliru, pastikan bot masih jadi member grup yang masih aktif."
            )
            # Tidak ada gunanya cek syarat lain — chat-nya sendiri tidak valid.
            return False, blockers
        # Error lain (network, dll) — tidak blokir, biarkan syarat lain tetap dicek.
