...
async def handle_fsm_input(client, message: Message):
    ...
    # ── Title VIP FSM ──────────────────────────────────────────────────────────
    from plugins.ui.handlers_dm import _vip_title_fsm
    if user_id in _vip_title_fsm:
        await _handle_vip_title_input(client, message, user_id)
        return
...
async def _handle_free_input(client, message: Message, user_id: int, state: dict):
    ...
    try:
        ...
        pass
    # Title VIP: pasang tag otomatis jika fitur aktif di grup ini.
    try:
        from core.vip_bio_guard import apply_vip_title
        await apply_vip_title(chat_id, target_id)
    except Exception:
        pass
...
# ─────────────────────────────────────────────────────────────────────────────
#  Handler input teks Title VIP (sub-menu panel Member VIP)
# ─────────────────────────────────────────────────────────────────────────────
async def _handle_vip_title_input(client, message: Message, user_id: int):
    """
    Input teks tag Title VIP — desain & validasi identik dengan Titel Admin
    NewsCore (maks 16 UTF-16 code unit, font unik/Unicode didukung), tapi
    disimpan ke key config "vip_title" (bukan newscore_config), dan begitu
    tersimpan langsung disinkronkan ke SEMUA Member VIP existing via
    sync_vip_titles() — bukan menunggu periode reset apapun.
    """
    from plugins.ui.handlers_dm import _vip_title_fsm
    from database import update_config

    state   = _vip_title_fsm[user_id]
    chat_id = state["chat_id"]
    ...
    try:
        if not text:
            await _safe_edit_id(
                ...
                "❌ Teks tidak boleh kosong.\n"
                "Ketik tag yang akan dipasang ke semua Member VIP.\n\n"
                "<i>Maksimal 16 karakter (font unik/Unicode didukung).</i>\n\n"
                "<i>Ketik /batal untuk membatalkan.</i>",
                InlineKeyboardMarkup([[InlineKeyboardButton("🚫 Batal", callback_data=f"viptitle_panel_{chat_id}")]]),
            )
            return
        ...
        await update_config(chat_id, "vip_title", text)
        await update_config(chat_id, "vip_title_enabled", True)
        _vip_title_fsm.pop(user_id, None)

        await _safe_edit_id(
            client, message.chat.id, msg_id,
            "✅ <b>Tag Title VIP disimpan!</b>\n\n"
            f"<code>{_html_escape(text)}</code>\n\n"
            "<i>Memasang tag ke semua Member VIP existing…</i>",
        )

        # Sinkronkan ke semua Member VIP existing dengan tag baru ini.
        try:
            from core.vip_bio_guard import sync_vip_titles
            await sync_vip_titles(chat_id)
        except Exception as e:
            print(f"[viptitle_input] sync_vip_titles error: {e}")

        await asyncio.sleep(1.0)
        from plugins.ui.pages import page_vip_title
        text_panel, keyboard_panel = await page_vip_title(chat_id)
        await _safe_edit_id(client, message.chat.id, msg_id, text_panel, keyboard_panel)

    except Exception as e:
        _vip_title_fsm.pop(user_id, None)
        print(f"[viptitle_input] {e}")
